...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Debugging: Print user IDs to ensure correct assignment
        print("User IDs:", [str(user._id) for user in users])

        # Create teams
        team1 = Team(_id=ObjectId(), name='Blue Team', members=[user._id for user in users])
        team1.save()

        team2 = Team(_id=ObjectId(), name='Gold Team', members=[user._id for user in users])
        team2.save()
